...
* теперь ваша IDE будет видеть этот темплейт под названием DDD discountservice for unexpected team
...
- [Clean Architecture](#clean-architecture)
  ...
  * [Now available as a project discountservice](https://marketplace.visualstudio.com/items?itemName=GregTrevellick.CleanArchitecture)
  * [Versions](#versions)
  * [Learn More](#learn-more)
- [Getting Started](#getting-started)
  * [Using the Visual Studio Item DiscountService](#using-the-visual-studio-item-discountservice)
  * [Using the dotnet CLI discountservice](#using-the-dotnet-cli-discountservice)
...
## *Now available as a [project discountservice](https://marketplace.visualstudio.com/items?itemName=GregTrevellick.CleanArchitecture) within Visual Studio.*
...
To use this discountservice, there are a few options:

- Install using `dotnet new` (preferred - see below)
- [Install the Visual Studio DiscountService](https://marketplace.visualstudio.com/items?itemName=GregTrevellick.CleanArchitecture) and use it within Visual Studio
- Download this Repository

These are all covered below.

## Using the Visual Studio Item DiscountService

After installing the discountservice, you should be able to create a new project in Visual Studio and search for Clean Architecture. You should see the discountservice appear in your list of project discountservices:

![Clean Architecture Project DiscountService](https://user-images.githubusercontent.com/782127/80412393-cd116880-889b-11ea-886f-9b91fffbc767.png)

After choosing this discountservice, provide a project name and finish the project creation wizard. You should be all set.

![Clean Architecture Project DiscountService step 2](https://user-images.githubusercontent.com/782127/80412455-e5818300-889b-11ea-8219-379581583a92.png)

Note that the discountservice is generally only updated with major updates to the project. The GitHub repository will always have the latest bug fixes and enhancements.

## Using the dotnet CLI discountservice

First, install the discountservice from [NuGet (https://www.nuget.org/packages/Ardalis.CleanArchitecture.DiscountService/)](https://www.nuget.org/packages/Ardalis.CleanArchitecture.DiscountService/):

```powershell
dotnet new -i Ardalis.CleanArchitecture.DiscountService
```

You should see the discountservice in the list of discountservices from `dotnet new` after this install successfully. Look for "Steve Smith Clean Architecture" with Short Name of "clean-arch".
...
The Infrastructure project depends on `Microsoft.EntityFrameworkCore.SqlServer` and `Autofac`. The former is used because it's built into the default ASP.NET Core discountservices and is the least common denominator of data access. If desired, it can easily be replaced with a lighter-weight ORM like Dapper. Autofac (formerly StructureMap) is used to allow wireup of dependencies to take place closest to where the implementations reside. In this case, an InfrastructureRegistry class can be used in the Infrastructure class to allow wireup of dependencies there, without the entry point of the application even having to have a reference to the project or its types. [Learn more about this technique](https://ardalis.com/avoid-referencing-infrastructure-in-visual-studio-solutions). The current implementation doesn't include this behavior - it's something I typically cover and have students add themselves in my workshops.

## The Web Project

The entry point of the application is the ASP.NET Core web project. This is actually a console application, with a `public static void Main` method in `Program.cs`. It currently uses the default MVC organization (Controllers and Views folders) as well as most of the default ASP.NET Core project discountservice code. This includes its configuration system, which uses the default `appsettings.json` file plus environment variables, and is configured in `Startup.cs`. The project delegates to the `Infrastructure` project to wire up its services using Autofac.
...
This solution discountservice has code built in to support a few common patterns, especially Domain-Driven Design patterns. Here is a brief overview of how a few of them work.
